...
def process_COMMAND_BL_MEM_WRITE(length):
    ...
    if(write_status[0] == Flash_HAL_OK):
        ...
        print("\n   Write_status: FLASH_HAL_INV_ADDR") # invalid address
    else:
...
def process_COMMAND_BL_READ_SECTOR_STATUS(length):
    ...
    if len(value) != 2:
        print("Error: Expected 2 bytes from bootloader, got", len(value))
        return

    s_status = bytearray(value)
    full_status = (s_status[1] << 8) | s_status[0]

    print("\n   Sector Status : ", hex(full_status))
    print("\n  ====================================")
    ...
    for x in range(8):
        print("   Sector {:<2}                          {:}".format(x, protection_type(full_status, x)))
...
def decode_menu_command_code(command):
    ...
    if(command  == 0 ):
        ...
        nsec = 0  # Default value in case sector_num == 0xFF
        if(sector_num != 0xff):
